import { useGetFeeEstimateQuery } from '@dogechia-network/api-react';
...
import mojoToDogechiaLocaleString from '../../utils/mojoToDogechiaLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendXDG = 'send_xdg_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToDogechiaLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const xdgFee = mojoToDogechiaLocaleString(estimate.estimate, 'en-US');
          setSelectedValue(estimate.formattedEstimate);
          setValue(name, xdgFee);
        }
      }
    }
