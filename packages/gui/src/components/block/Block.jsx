...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@dogechia-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToDogechia(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToDogechia(calculateBaseFarmerReward(blockRecord.height));

  const dogechiaFees = blockRecord.fees !== undefined ? mojoToDogechia(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: dogechiaFees ? `${dogechiaFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the Dogechia blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
