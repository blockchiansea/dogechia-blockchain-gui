...
import { useCreateOfferForIdsMutation, useGetWalletBalanceQuery } from '@dogechia-network/api-react';
import {
  Amount,
  AmountProps,
  Back,
  ...
  mojoToDogechia,
  mojoToDogechiaLocaleString,
  useColorModeValue,
  useCurrencyCode,
  useLocale,
  useOpenDialog,
  useShowError,
} from '@dogechia-network/core';
...
/*       An NFT offer consists of a single NFT being offered for XDG/CAT      */
/* ========================================================================== */
          ...
          <Box style={{ position: 'relative', top: '20px' }}>
            <TooltipIcon>
              <Trans>
                ...
                recommended minimum fee is 0.000005 XDG (5,000,000 mojos)
              </Trans>
            </TooltipIcon>
          </Box>
        </Flex>
    ...
    if (
      ...
      switch (tokenWalletInfo.walletType) {
        case WalletType.STANDARD_WALLET:
          balanceString = mojoToDogechiaLocaleString(walletBalance.spendableBalance, locale);
          balance = mojoToDogechia(walletBalance.spendableBalance);
          break;
        case WalletType.CAT:
...
/* ========================================================================== */
/*                              NFT Offer Editor                              */
/*           Currently only supports a single NFT <--> XDG/CAT offer          */
/* ========================================================================== */
...
function buildOfferRequest(params: NFTBuildOfferRequestParams) {
  ...
  const baseMojoAmount: BigNumber =
    tokenWalletInfo.walletType === WalletType.CAT ? catToMojo(tokenAmount) : dogechiaToMojo(tokenAmount);
  const mojoAmount = exchangeType === NFTOfferExchangeType.NFTForToken ? baseMojoAmount : baseMojoAmount.negated();
  const feeMojoAmount = dogechiaToMojo(fee);
