...
import type { CATToken, Wallet } from '@dogechia-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@dogechia-network/api-react';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const dogechiaWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Dogechia',
        symbol: currencyCode,
        displayName: `Dogechia (${currencyCode})`,
        disabled: false,
        tail: '',
      })
    );
    ...
    const allOptions = [...dogechiaWalletSelection, ...catOptions];
