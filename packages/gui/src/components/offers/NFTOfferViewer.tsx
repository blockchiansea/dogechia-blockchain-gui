import { OfferSummaryRecord, OfferTradeRecord } from '@dogechia-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@dogechia-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['xdg', 'txdg'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.DOGECHIA: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const xdgMakerFee = mojoToDogechia(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(xdgMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.DOGECHIA
        ? dogechiaToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
