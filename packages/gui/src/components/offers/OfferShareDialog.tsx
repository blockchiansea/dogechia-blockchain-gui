import { OfferTradeRecord } from '@dogechia-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('dogechia-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.dogechi.org';
...
enum HashgreenErrorCodes {
  ...
  MARKET_NOT_FOUND = 50_029, // Pairing doesn't exist e.g. XDG/RandoCoin
...
async function postToHashgreen(offerData: string, testnet: boolean): Promise<string> {
  ...
  if (statusCode === 200) {
    ...
    if (testnet) {
      return 'https://www.dogechi.org/offers';
    }
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'txdg' : 'xdg'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
