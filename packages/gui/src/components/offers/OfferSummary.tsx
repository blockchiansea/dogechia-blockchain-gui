import { type OfferSummaryRecord } from '@dogechia-network/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToDogechia, mojoToCAT } from '@dogechia-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xdg', 'txdg'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToDogechia(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xdg', 'txdg'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToDogechia(takerEntries[0][1])
