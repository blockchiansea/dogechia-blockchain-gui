...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@dogechia-network/api';
import { mojoToCAT, mojoToDogechia, mojoToCATLocaleString, mojoToDogechiaLocaleString } from '@dogechia-network/core';
...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> XDG assumption
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToDogechiaLocaleString(amount, locale);
  }
...
export function offerAssetTypeForAssetId(assetId: string, offerSummary: OfferSummaryRecord): OfferAsset | undefined {
  ...
  if (['xdg', 'txdg'].includes(assetId)) {
    assetType = OfferAsset.DOGECHIA;
  } else {
    const { infos } = offerSummary;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function offerAssetIdForAssetType(
  ...
  if (assetType === OfferAsset.DOGECHIA) {
    return keys.includes('xdg') ? 'xdg' : undefined;
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.DOGECHIA]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.DOGECHIA ? mojoToDogechia(amountInMojos) : mojoToCAT(amountInMojos);
