import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@dogechia-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@dogechia-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@dogechia-network/api-react';
import { catToMojo, dogechiaToMojo } from '@dogechia-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedXDG = useWatch({
    name: 'requested.xdg',
  });

  const offeredXDG = useWatch({
    name: 'offered.xdg',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedXDG ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('xdg', dogechiaToMojo(amount))),
    ...(requestedTokens ?? [])
