import { Flex } from '@dogechia-network/core';
import { Offering, Requesting } from '@dogechia-network/icons';
...
import OfferBuilderXDGSection from './OfferBuilderXDGSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showXDG = !readOnly || !!xdg.length;
  ...
  const mutedXDG = nfts.length || tokens.length;
  const mutedTokens = xdg.length || nfts.length;
  const mutedNFTs = xdg.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showXDG && <OfferBuilderXDGSection name={`${name}.xdg`} offering={offering} muted={mutedXDG} />}
