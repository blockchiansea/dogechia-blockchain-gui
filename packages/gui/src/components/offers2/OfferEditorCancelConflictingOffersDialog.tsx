import { ConfirmDialog, Flex } from '@dogechia-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'XDG' && assetStatus.type === 'XDG') {
      // assetId might be either 'XDG' or 'UNKNOWN'(maybe fee) but here both are treated as just a XDG spending
