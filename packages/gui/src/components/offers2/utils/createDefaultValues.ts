...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (XDG), indicates whether a token or CAT has a default entry
 ...
 * for creating an XDG offer, a CAT offer, or an NFT offer. XDG and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      xdg: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
