...
} from '@dogechia-network/api-react';
import {
  ButtonLoading,
  EstimatedFee,
  FeeTxType,
...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'XDG').toUpperCase();
  const isTestnet = currencyCode === 'TXDG';
  ...
  const spendableBalance = mojoToDogechiaLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet10-faucet.dogechi.org' : 'faucet.dogechi.org'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Dogechia Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
