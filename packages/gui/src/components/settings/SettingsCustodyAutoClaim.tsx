import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@dogechia-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, dogechiaToMojo, mojoToDogechia } from '@dogechia-network/core';
import { ConnectCheckmark } from '@dogechia-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToDogechia(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = dogechiaToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
