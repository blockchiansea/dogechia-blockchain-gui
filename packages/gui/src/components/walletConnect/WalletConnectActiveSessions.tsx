import { Flex, Loading, TooltipIcon } from '@dogechia-network/core';
...
const supportedCommands = WalletConnectCommands.map((item) => `dogechia_${item.command}`);
        ...
        {isLoading ? (
          ...
          <Flex flexDirection="column" gap={2}>
            {pair.sessions.map((session) => {
              ...
              const methods = namespaces.dogechia?.methods ?? [];
                ...
                <Flex flexDirection="column">
                  ...
                  {unsupportedMethods.length ? (
                    <Typography variant="body2" color="textSecondary">
                      ...
                      <TooltipIcon>
                        <Trans>
                          Your current version of the Dogechia application may not support some commands requested by the
