import { Button, Flex, useOpenDialog, dogechiaToMojo } from '@dogechia-network/core';
...
export default function WalletConnectOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderData) {
      // use new fee value
      const feeDogechia = offerBuilderData.offered.fee?.[0]?.amount;
      if (feeDogechia) {
        const feeMojos = feeDogechia ? dogechiaToMojo(feeDogechia).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
