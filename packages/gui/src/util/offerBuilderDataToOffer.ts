...
import { dogechiaToMojo, catToMojo } from '@dogechia-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { xdg: offeredXdg = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { xdg: requestedXdg = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? dogechiaToMojo(firstFee.amount) : new BigNumber(0);

  const walletIdsAndAmounts: Record<string, BigNumber> = {};
  const driverDict: Record<string, Driver> = {};

  if (!allowEmptyOfferColumn) {
    const hasOffer = !!offeredXdg.length || !!offeredTokens.length || !!offeredNfts.length;
        ...
        if (assetId.toUpperCase() === 'XDG' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have xdg spending which is only XDG, only Fee or both XDG and Fee
              pendingOffers[idx].assetId = 'XDG+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'XDG',
  ...
  let pendingXdgOffer: AssetStatusForOffer | undefined;
  let pendingXdg = new BigNumber(0);
  if (offeredXdg.length > 0 || feeInMojos.gt(0)) {
  ...
  // offeredXdg.length should be always 0 or 1
  const xdgTasks = offeredXdg.map(async (xdg) => {
    const { amount } = xdg;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an XDG amount`);
    }
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }

    const mojoAmount = dogechiaToMojo(amount);
    ...
    const hasEnoughTotalBalance = spendableBalance.plus(pendingXdg).minus(feeInMojos).gte(mojoAmount);
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds XDG total balance`);
    }

    if (pendingXdgOffer) {
      // Assuming offeredXdg.length is always less then or equal to 1
      pendingXdgOffer.spendingAmount = mojoAmount.plus(feeInMojos);
      pendingXdgOffer.spendableAmount = spendableBalance;
      pendingXdgOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingXdgOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
        pendingXdgOffer.status = 'conflictsWithNewOffer';
      } else {
        pendingXdgOffer.status = 'alsoUsedInNewOfferWithoutConflict';
      }
    }
  });
  // Treat fee as xdg spending
  if (offeredXdg.length === 0 && feeInMojos.gt(0)) {
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds XDG total balance`);
    }
    if (pendingXdgOffer) {
      pendingXdgOffer.spendingAmount = feeInMojos;
      pendingXdgOffer.spendableAmount = spendableBalance;
      pendingXdgOffer.confirmedAmount = new BigNumber(standardWalletBalance.confirmedWalletBalance);
      const hasEnoughSpendableBalance = spendableBalance.gte(pendingXdgOffer.spendingAmount);
      if (!hasEnoughSpendableBalance) {
  ...
  await Promise.all([...xdgTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedXdg.forEach((xdg) => {
    const { amount } = xdg;
    ...
    walletIdsAndAmounts[wallet.id] = dogechiaToMojo(amount);
  });
  ...
  const prepareNftTasks = requestedNfts.map(async ({ nftId }) => {
    ...
    if (driver) {
      ...
      if (considerNftRoyalty && pendingXdgOffer) {
        ...
        if (royaltyPercentageStr) {
          ...
          const spendingXdg = pendingXdgOffer.spendingAmount.minus(feeInMojos);
          const newSpendingXdg = spendingXdg.multipliedBy(royaltyMultiplier).plus(feeInMojos);
          pendingXdgOffer.spendingAmount = newSpendingXdg;

          const hasEnoughSpendableBalance = pendingXdgOffer.spendableAmount.gte(pendingXdgOffer.spendingAmount);
          if (!hasEnoughSpendableBalance) {
            pendingXdgOffer.status = 'conflictsWithNewOffer';
          } else {
            pendingXdgOffer.status = 'alsoUsedInNewOfferWithoutConflict';
