...
export function getUserDataDir(): string {
  const dogechiaRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(dogechiaRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const dogechiaRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${dogechiaRootUserDataPath}`);
  app.setPath('userData', dogechiaRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(dogechiaRootUserDataPath, 'Local Storage', 'leveldb');
