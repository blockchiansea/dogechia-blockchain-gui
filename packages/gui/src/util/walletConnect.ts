...
const log = initDebug('dogechia-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `dogechia_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.dogechia;
    if (!requiredNamespace) {
      throw new Error('Missing required dogechia namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['dogechia:testnet', 'dogechia:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `dogechia:${instance}:${fingerprint}`);
