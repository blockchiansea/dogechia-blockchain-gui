import { TransactionType, WalletType } from '@dogechia-network/api';
import type { Transaction } from '@dogechia-network/api';
import { useGetWalletBalanceQuery } from '@dogechia-network/api-react';
import { mojoToDogechia, mojoToCAT, blockHeightToTimestamp } from '@dogechia-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToDogechia(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: (walletType === WalletType.CAT ? mojoToCAT(balance) : mojoToDogechia(balance)).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, (walletType === WalletType.CAT ? mojoToCAT(start) : mojoToDogechia(start)).toNumber()), // max 21,000,000 safe to number
      tooltip: walletType === WalletType.CAT ? mojoToCAT(start) : mojoToDogechia(start).toString(), // bignumber is not supported by react
    });
  });
