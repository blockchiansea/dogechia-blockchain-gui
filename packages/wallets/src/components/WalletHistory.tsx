import { WalletType, TransactionType, toBech32m } from '@dogechia-network/api';
...
import type { Row } from '@dogechia-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          &nbsp;
          <strong>
            <FormatLargeNumber value={type === WalletType.CAT ? mojoToCAT(row.amount) : mojoToDogechia(row.amount)} />
          </strong>
          &nbsp;
          {metadata.unit}
        </>
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToDogechia(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
