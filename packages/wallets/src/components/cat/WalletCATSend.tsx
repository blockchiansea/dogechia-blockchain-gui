import { SyncingStatus, toBech32m } from '@dogechia-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@dogechia-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send dogechia to coloured address. Please enter a dogechia address.`);
    }

    if (address.includes('dogechia_addr') || address.includes('colour_desc')) {
    ...
    const feeValue = dogechiaToMojo(fee);
