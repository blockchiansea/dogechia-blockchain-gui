...
import { AlertDialog } from '@dogechia/core';
import {
  Typography,
  Button,
  Box,
...
import { dogechia_to_mojo } from '../../../util/dogechia';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      dogechiaper_input.value === '' ||
      Number(dogechiaper_input.value) === 0 ||
      !Number(dogechiaper_input.value) ||
      isNaN(Number(dogechiaper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const dogechiaper = dogechia_to_mojo(dogechiaper_input.value);
    const dogechiaper_value = Number.parseInt(Number(dogechiaper));
    ...
    // var fee = dogechia_to_mojo(fee_input.value);
