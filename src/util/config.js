...
global.daemon_rpc_ws = `wss://${self_hostname}:56669`;
...
function loadConfig(net) {
  try {
    // check if DOGECHIA_ROOT is set. it overrides 'net'
    const config_root_dir =
      'DOGECHIA_ROOT' in process.env
        ? process.env.DOGECHIA_ROOT
        : path.join(os.homedir(), '.dogechia', net);
    ...
    const daemon_port = lodash.get(config, 'ui.daemon_port', 56669); // jshint ignore:line
