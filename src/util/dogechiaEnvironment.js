...
const PY_FOLDER = '../dogechia/daemon';
...
const getDogeChiaVersion = () => {
  let version = null;
  const exePath = getExecutablePath('dogechia');
  // first see if we can get a dogechia exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or dogechia is on the path
    try {
      version = child_process
